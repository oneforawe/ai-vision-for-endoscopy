#!/usr/bin/env python3
# filename: presentation_make_cm.py

# Run in `source` folder with the following command:
# python -m offline-eval.presentation_make_cm

import os
import pandas as pd
import presentation_figures as pres_figs
...
def main():
    ...
    eval_data_paths.append( {'model_short_name' : 'MNv2d',
        'data_short_name' : 'C',
        'run' : 16,
        'filepath' :
        '../output/offline-eval/MNv2d/2-processed/by_abnorm/data_C/Run_16/' +
        'evaluations/MNv2dCr16_eval_metrics.csv'} )
    eval_data_paths.append( {'model_short_name' : 'MNv2a',
        'data_short_name' : 'E',
        'run' : 2,
        'filepath' :
        '../output/offline-eval/MNv2a/2-processed/by_abnorm/data_E/Run_02/' +
        'evaluations/MNv2aEr2_eval_metrics.csv'} )
    eval_data_paths.append( {'model_short_name' : 'MNv2d',
        'data_short_name' : 'E',
        'run' : 1,
        'filepath' :
        '../output/offline-eval/MNv2d/2-processed/by_abnorm/data_E/Run_01/' +
        'evaluations/MNv2dEr1_eval_metrics.csv'} )
    ...
    reckoning_paths.append( {'filepath' :
        '../output/offline-eval/MNv2d/2-processed/by_abnorm/data_C/Run_16/'+
        'evaluations/MNv2dCr16_test_w_reckoning_choices.csv'} )
    reckoning_paths.append( {'filepath' :
        '../output/offline-eval/MNv2a/2-processed/by_abnorm/data_E/Run_02/'+
        'evaluations/MNv2aEr2_test_w_reckoning_choices.csv'} )
    reckoning_paths.append( {'filepath' :
        '../output/offline-eval/MNv2d/2-processed/by_abnorm/data_E/Run_01/'+
        'evaluations/MNv2dEr1_test_w_reckoning_choices.csv'} )
    ...
    for run_eval, run_reckon in zip(eval_data_paths, reckoning_paths):
        ...
        eval_fig_path = eval_path + 'figures/presentation_2/'
        os.makedirs(eval_fig_path, exist_ok=True)

        # Make plots and data.  (thresh, CM figs, and reckonings)
        pres_figs.pick_thresh_make_figures(evaluations,
