...
def make_acc_loss_plots(histories, eval_fig_path, plot_run_name):
    ...
    for plot_this in plot_these:
        ...
        for i in range(len(histories)):
            #if i==3:
            if i==2:
                legend_labels.append(f'train')
        for i in range(len(histories)):
            #legend_labels.append(f'validation f{i+1}')
            #if i==3:
            if i==2:
                legend_labels.append(f'valid.')
        if plot_this[0]=='acc':
            plt.legend(legend_labels, loc='upper right')
            #plt.legend(legend_labels, loc='center right')
...
def make_roc_plot(test_set, eval_fig_path, plot_run_name):
    ...
    plt.plot(fpr, tpr, color='darkorange',
             lw=lw, label=f'(area = {roc_auc:0.2f})')
...
def make_eval_metric_figures(test_w_reckonings, thresh,
    ...
    plot_confusion_matrix(cnf_matrix, classes=class_names,
                          title=f'Confusion matrix (T={thresh:0.1f})')
    savefig(eval_fig_path
            +f'{plot_run_name}_Confusion_Matrix_T_{thresh:0.1f}.png',
