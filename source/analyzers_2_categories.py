...
def train_model(input_model, batch_size, epochs, img_size,
    ...
    os.makedirs(f'./chkpts/{model.name}/{file_v}', exist_ok=True)
    os.makedirs(f'./tb_logs/{model.name}/{file_v}',exist_ok=True)

    print(f'\n')
    print(f'Training: Run {j:02d}')
    print(f'----------------\n\n')
    os.makedirs(f'./chkpts/{model.name}/{file_v}/Run_{j:02d}', exist_ok=True)  # for /weights_fold_{str(i)}.hdf5
    os.makedirs(f'./tb_logs/{model.name}/{file_v}/Run_{j:02d}',exist_ok=True)  # for /tb_fold_{str(i)}/etcetera
    ...
    for train_index, valid_index in kf.split(x):
        ...
        os.makedirs(f'./tb_logs/{model.name}/{file_v}/Run_{j:02d}/tb_fold_{str(i)}',exist_ok=True)
        ...
        callbacks = [EarlyStopping(monitor='val_loss', patience=3, verbose=1, min_delta=1e-4),
                     ...
                     ModelCheckpoint(filepath =
                                     f'./chkpts/{model.name}/{file_v}/Run_{j:02d}/weights_fold_{str(i)}.hdf5',
                                     verbose=1, save_best_only=True, save_weights_only=True, mode='auto'),
                     TensorBoard(log_dir = f'./tb_logs/{model.name}/{file_v}/Run_{j:02d}/tb_fold_{str(i)}/' )
                    ]
        ...
        model.load_weights(filepath = f'./chkpts/{model.name}/{file_v}/Run_{j:02d}/weights_fold_{str(i)}.hdf5' )
    ...
    # Save
    model.save(f'./chkpts/{model.name}/{file_v}/Run_{j:02d}/ModelWhole_trained_{model.name}_{file_v}_Run_{j:02d}.hdf5')
    #model = load_model('my_model.hdf5')
    model_json_string = model.to_json()
    with open(f'./chkpts/{model.name}/{file_v}/Run_{j:02d}/ModelArch_{model.name}_.json', "w") as json_file:
        json_file.write(model_json_string)
    #model = model_from_json(json_string)
    model.save_weights(f'./chkpts/{model.name}/{file_v}/Run_{j:02d}/ModelWeights_trained_{model.name}_{file_v}_Run_{j:02d}.hdf5')
